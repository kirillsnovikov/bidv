...
SOAPClient._onLoadWsdl = function(
  url,
  method,
  parameters,
  async,
...
SOAPClient._sendSoapRequest = function(
  url,
  method,
  parameters,
  async,
  ...
  xmlHttp.send(sr);
  if (!async)
    return SOAPClient._onSendSoapRequest(
      method,
      async,
      callback,
      wsdl,
...
SOAPClient._node2object = function(node, wsdl) {
  ...
  var isarray =
    SOAPClient._getTypeFromWsdl(node.nodeName, wsdl)
      .toLowerCase()
      .indexOf("arrayof") != -1;
  // object node
  if (!isarray) {
...
SOAPClient._extractValue = function(node, wsdl) {
  ...
  switch (
    SOAPClient._getTypeFromWsdl(node.parentNode.nodeName, wsdl).toLowerCase()
  ) {
    default:
    case "s:string":
...
SOAPClient._getTypeFromWsdl = function(elementname, wsdl) {
  ...
  if (ell.length == 0) ell = wsdl.getElementsByTagName("element"); // MOZ
  for (var i = 0; i < ell.length; i++) {
    if (ell[i].attributes["name"] + "" == "undefined") {
      // IE
      if (
        ell[i].attributes.getNamedItem("name") != null &&
        ell[i].attributes.getNamedItem("name").nodeValue == elementname &&
      ...
      if (
        ...
        ell[i].attributes["name"].value == elementname &&
